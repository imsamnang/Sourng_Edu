...
{
    protected $base_route = 'student.document';
    protected $view_path = 'student.document';
    protected $panel = '';
    ...
    {
        $this->folder_path = public_path().DIRECTORY_SEPARATOR.$this->folder_name.DIRECTORY_SEPARATOR.'student'.DIRECTORY_SEPARATOR;
    }
    ...
    public function delete(Request $request, $id)
    {
        ...
        $reg_no = parent::getStudentById($row->students_id);
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    switch ($request->get('bulk_action')) {
                        ...
                        case 'delete':
                            $row = Document::find($row_id);
                            $reg_no = Student::select('reg_no')->where('id','=',$row->member_id)->first();
                            // remove old image from folder
