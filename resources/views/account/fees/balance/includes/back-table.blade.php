    ...
    <div class="col-xs-12">
        ...
        <div class="clearfix">
            <span class="form-group due-reminder-submit">
                    <label>{!! Form::radio('type[]','sms' ,false, ["class" => "ace", "id"=>"typeSms"]) !!}<span class="lbl"> {{ __('acount-fee-balance.SMS') }} </span></label>
                    <label>{!! Form::radio('type[]','email' ,false, ["class" => "ace", "id"=>"typeEmail"]) !!}<span class="lbl">{{ __('acount-fee-balance.E-mail') }}</span></label>
                    <a class="btn-primary btn-sm message-send-btn" ><i class="fa fa-envelope" aria-hidden="true"></i>&nbsp;{{ __('acount-fee-balance.SendReminder') }}</a>
                    <a class="btn-success btn-sm bulk-due-slip" >{{ __('acount-fee-balance.BulkDueDetail') }} <i class="fa fa-print" aria-hidden="true"></i></a>
            </span>
        ...
        <div class="table-responsive">
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                ...
                <tr>
                    ...
                    <th>{{ __('acount-fee-balance.S.N.') }}</th>
                    <th>{{ __('acount-fee-balance.Faculty') }}</th>
                    <th>{{ __('acount-fee-balance.Sem.') }}</th>
                    <th>{{ __('acount-fee-balance.Reg.Number') }}</th>
                    <th>{{ __('acount-fee-balance.NameofStudent') }}</th>
                    <th>{{ __('acount-fee-balance.TotalFee') }}</th>
                    <th>{{ __('acount-fee-balance.Balance') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
