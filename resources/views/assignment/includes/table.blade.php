<div class="row">
    <div class="col-xs-12">
        <h4 class="header large lighter blue"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;{{ __('assignment.AssignmentList') }}</h4>
        <div class="clearfix">

    <span class="easy-link-menu">
        <a class="btn-primary btn-sm bulk-action-btn" attr-action-type="active"><i class="fa fa-check" aria-hidden="true"></i>&nbsp;{{ __('assignment.Active') }}</a>
        <a class="btn-warning btn-sm bulk-action-btn" attr-action-type="in-active"><i class="fa fa-remove" aria-hidden="true"></i>&nbsp;{{ __('assignment.In-Active') }}</a>
        <a class="btn-danger btn-sm bulk-action-btn" attr-action-type="delete"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;{{ __('assignment.Delete') }}</a>
    </span>

            <span class="pull-right tableTools-container"></span>
        </div>
        <div class="table-header">
            {{ $panel }}  {{ __('Panel_st_Record') }}
        </div>
        ...
        <div class="table-responsive">
            ...
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                ...
                <tr>
                    ...
                    <th>{{ __('assignment.S.N.') }}</th>
                    <th>{{ __('assignment.SEM/SEC') }}</th>
                    <th width="40%">{{ __('assignment.Question') }}</th>
                    <th>{{ __('assignment.AvailableOn') }}</th>
                    <th>{{ __('assignment.Submit') }}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <a href="{{ route('assignment.view', ['id' => Crypt::encryptString($assignment->id)]) }}" class="btn btn-primary btn-minier btn-primary">
                                        <i class="ace-icon fa fa-eye bigger-130"></i>
                                    </a>

                                    <a href="{{ route('assignment.edit', ['id' => Crypt::encryptString($assignment->id)]) }}" class="btn btn-primary btn-minier btn-success">
                                        <i class="ace-icon fa fa-pencil bigger-130"></i>
                                    </a>

                                    <a href="{{ route('assignment.delete', ['id' => Crypt::encryptString($assignment->id)]) }}" class="btn btn-primary btn-minier btn-danger bootbox-confirm" >
