      ...
      <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $i }}</td>
            <td><a href="{{ route('student.view', ['id' => $document->member_id]) }}"> {{  ViewHelper::getStudentById( $document->member_id ) }}</a></td>
            <td>
            {{--<a href="{{ asset('documents'.DIRECTORY_SEPARATOR.'student'.DIRECTORY_SEPARATOR.ViewHelper::getStudentById( $document->member_id ).'/'.$document->file) }}" target="_blank">
                <i class="ace-icon fa fa-download bigger-120"></i> &nbsp;{{ $document->title }}
              </a> --}}
              <a href="{{route('student.document.download',[ViewHelper::getStudentById( $document->member_id ),$document->id])}}" target="_blank">
                <i class="ace-icon fa fa-download bigger-120"></i> &nbsp;{{ $document->title }}
              </a>              
            </td>
            <td><strong>{{$document->download_count}} times</strong></td>
            <td class="hidden-480 ">
              <div class="btn-group">
            ...
            <td>
              <div class="hidden-sm hidden-xs action-buttons">
                ...
                <a href="{{ route($base_route.'.edit', ['id' => $document->id]) }}" class="btn btn-primary btn-minier btn-success">
                  <i class="ace-icon fa fa-pencil bigger-130"></i>
                </a>

                <a href="{{ route($base_route.'.delete', ['id' => $document->id]) }}" class="btn btn-primary btn-minier btn-danger bootbox-confirm" >
