                ...
                @if($allQuiz)
                  @foreach($allQuiz as $quiz)
                    <tr>
                      ...
                      <td>{{ $quiz->title }}</td>
                      <td>
                        <a href="{{ route('quiz.subject.show',$quiz->slug) }}" class="btn btn-success btn-xs btn-minier">View Quiz</a>
                      </td>
                      <td>
                        <a href="{{ route('quiz.question.create',$quiz->slug) }}" class="btn btn-info btn-xs btn-minier">Add Questions</a>
                      </td>
                      <td>
                      ...
                      <td>
                        <form method="POST" action="{{route('quiz.subject.destroy',$quiz->id)}}" accept-charset="UTF-8" style="display:inline" id="deleteObject-{{$quiz->id}}">
                          {{csrf_field()}}
                          {{method_field('DELETE')}}
                          <a href="#" class="btn btn-danger btn-xs btn-minier" onclick="deleteObject({{$quiz->id}})">Remove
                          </a>
                        </form>                            
                      </td>
                    </tr>
